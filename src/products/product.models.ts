...
export const CategoryModel = sequelize.define<CategoryModelInterface>(
  "Category",
  {
    name: {
...
export const ProductImagesModel = sequelize.define<ProductImagesModelInterface>(
  "ProductImages",
  {
    image: {
      type: DataTypes.STRING,
...
export const LikedProductModel = sequelize.define<LikedProductModelInterface>(
  "LikedProduct",
  {
    productId: {
      type: DataTypes.INTEGER,
  ...
  async findById(id: number) {
    ...
    if (product) {
      const imageURLs = product.images.map(
        (image: ProductImagesModelInterface) => {
  ...
  async findByCategory(categoryId: number) {
    ...
    return products.map((product) => ({
      ...
      imageURL: product.images.length
        ? getFileURL(product.images[0].image)
        : "", // Add the image URL
    }));
  }
}
...
export class LikedProductService {
  ...
  async likedProductByUser(
    userId: string
  ): Promise<LikedProductModelInterface[]> {
    const likedProducts = await LikedProductModel.findAll({
      where: { userId: userId },
    });
    return likedProducts.map((likedProduct: any) =>
      likedProduct.toJSON()
    ) as LikedProductModelInterface[];
  }
}
